...
def prepOneHotEncoder(df, col, pathPackages):
    oneHotEncoder = pickle.load(open(os.path.join(pathPackages, f'prep{col}.pkl'), 'rb'))
...
def prepStandardScaler(df, col, pathPackages):
    scaler = pickle.load(open(os.path.join(pathPackages, f'prep{col}.pkl'), 'rb'))
    df[col] = scaler.transform(df[[col]])
    return df

def runModel(data, path):
    pathPackages = os.path.join(path, "machine_learning", "packages")
